...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    if (!is_regular_file(exe))
    {
        ...
        LOG_CRIT("sql.updates", "Didn't find any executable MySQL binary at \'{}\' or in path, correct the path in the *.conf (\"MySQLExecutable\").",
...
// Auth Database
template<>
std::string DBUpdater<DiscordDatabaseConnection>::GetConfigEntry()
{
    return "Updates.Discord";
}

template<>
std::string DBUpdater<DiscordDatabaseConnection>::GetTableName()
{
    return "Discord";
}

template<class T>
std::string DBUpdater<T>::GetBaseFilesDirectory()
{
    return BuiltInConfig::GetSourceDirectory() + "/sql/base/db_" + GetLowerString(DBUpdater<T>::GetTableName()) + "/";
}

template<>
bool DBUpdater<DiscordDatabaseConnection>::IsEnabled(uint32 const updateMask)
{
...
template<>
std::string DBUpdater<DiscordDatabaseConnection>::GetDBModuleName()
{
    return "db_discord";
}
...
bool DBUpdater<T>::Create(DatabaseWorkerPool<T>& pool)
{
    ...
    if (!file.is_open())
    {
        LOG_CRIT("sql.updates", "Failed to create temporary query file \"{}\"!", temp.generic_string());
        return false;
    }
    ...
    catch (UpdateException const& /*ex*/)
    {
        LOG_CRIT("sql.updates", "Failed to create database {}! Does the user (named in *.conf) have `CREATE`, `ALTER`, `DROP`, `INSERT` and `DELETE` privileges on the MySQL server?", pool.GetConnectionInfo()->database);
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool, std::string_view modulesList /*= {}*/)
{
    ...
    {
        LOG_ERROR("sql.updates", "DBUpdater: The given source directory {} does not exist, change the path to the directory where your sql directory exists. Shutting down.",
    ...
    auto CheckUpdateTable = [&](std::string const& tableName)
    {
        ...
        if (!checkTable)
        {
            ...
            {
                LOG_CRIT("sql.updates", "Failed apply file to database {}! Does the user (named in *.conf) have `INSERT` and `DELETE` privileges on the MySQL server?", pool.GetConnectionInfo()->database);
                return false;
            }
                             ...
                             std::string const& password, std::string const& port_or_socket, std::string const& database, std::string const& ssl, Path const& path)
{
    ...
    if (ret != EXIT_SUCCESS)
    {
        LOG_CRIT("sql.updates", "Applying of file \'{}\' to database \'{}\' failed!" \
...
template class WH_DATABASE_API DBUpdater<DiscordDatabaseConnection>;
