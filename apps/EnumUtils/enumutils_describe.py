...
EnumClassPattern = compile(r'//\s*EnumUtils: DESCRIBE THIS(?:\s*\(in ([^\)]+)\))?\s+enum\s+class\s+([0-9A-Za-z]+)[^\n]*\s*{([^}]+)};')
...
def processFile(path, filename):
    ...
    for enum in EnumClassPattern.finditer(file):
        prefix = enum.group(1) or ''
        name = enum.group(2)
        values = []
    ...
    for name, prefix, values in enums:
        ...
        output.write('WH_API_EXPORT EnumText EnumUtils<%s>::ToString(%s value)\n' % (name, name))
        output.write('{\n')
        output.write('    switch (value)\n')
        output.write('    {\n')

        for label, title, description in values:
            output.write('        case %s::%s: return { %s, %s, %s };\n' % (prefix + name, prefix + label, strescape(label), strescape(title), strescape(description)))
        ...
        output.write('\n')
        output.write('template<>\n')
        output.write('WH_API_EXPORT size_t EnumUtils<%s>::Count() { return %d; }\n' % (name, len(values)))
        output.write('\n')
        output.write('template<>\n')
        output.write('WH_API_EXPORT %s EnumUtils<%s>::FromIndex(size_t index)\n' % (name, name))
        output.write('{\n')
        output.write('    switch (index)\n')
        output.write('    {\n')

        for (i, (label, title, description)) in enumerate(values):
            output.write('        case %d: return %s::%s;\n' % (i, prefix + name, prefix + label))
        ...
        output.write('\n')
        output.write('template<>\n')
        output.write('WH_API_EXPORT size_t EnumUtils<%s>::ToIndex(%s value)\n' % (name, name))
        output.write('{\n')
        output.write('    switch (value)\n')
        output.write('    {\n')

        for (i, (label, title, description)) in enumerate(values):
            output.write('        case %s::%s: return %d;\n' % (prefix + name, prefix + label, i))
